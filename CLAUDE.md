...
- `packages/cli/bin/latent.mjs` is the CLI. Every command supports `--json` (agent output is the primary consumer, human-readable text is secondary). Key mechanics:
  ...
  - `check-docs` runs two independent checks and reports one merged `violations` list, each entry tagged `kind`: `"stale-term"` or `"doc-schema"`.
    ...
    - `doc-schema`: loads every component's `*.doc.mjs` and enforces the shape described above — all seven top-level fields present, `summary`/`example` non-empty, `doNot` and `figmaTokens` non-empty (opt out of `figmaTokens` by omitting it, not by shipping `{}` — see `ERR_NO_FIGMA_SPEC`), and every entry in `props` has `name`/`type`/`default`/`description`. This was previously just a documented convention nothing enforced; it's now load-bearing the same way `check-parity` is for CSS/token drift.
