...
- `packages/cli/bin/latent.mjs` is the CLI. Every command supports `--json` (agent output is the primary consumer, human-readable text is secondary). Key mechanics:
  ...
  - `check-docs` runs five independent checks and reports one merged `violations` list, each entry tagged `kind`: `"stale-term"`, `"doc-schema"`, `"plugin-coverage"`, `"lockfile-sync"`, or `"icon-cascade"`.
    ...
    - `plugin-coverage` (added 2026-08-27): checks both directions of drift between `discoverComponents()`'s real component list and `packages/figma-plugin/code.js`'s own `COMPONENT_NAMES` array (parsed by regex, not imported — `code.js` runs inside the Figma plugin sandbox, not something this CLI can `import`). A component with a real `.doc.mjs` missing from `COMPONENT_NAMES` means the Latent Sync plugin's extraction silently produces no live-bindings data for it — indistinguishable from "brand new, never synced" without this check, since `check-component-bindings` reports both as the same non-blocking `"no-live-data"`. The reverse (a stale name still in `COMPONENT_NAMES` for a removed component) is flagged too, as harmless dead weight. Built after Alert/AlertStack/Select/MultiSelect/SelectOption shipped real `.doc.mjs` files but were never added to `COMPONENT_NAMES`, silently missing from `component-bindings.live.json` for the better part of a session.
    - `lockfile-sync` (added 2026-08-27): checks that every `packages/*/package.json` workspace has a matching `"packages/<dir>"` entry in the root `package-lock.json`'s `packages` map. `npm install` (used for everyday local work) tolerates a stale lockfile silently; `npm ci` (what `.github/workflows/latent-sync-check.yml` actually runs) fails hard on the mismatch with no local warning otherwise. Built after `packages/gallery` was tracked as a real workspace without regenerating `package-lock.json`, which broke CI's `npm ci` step with zero signal beforehand.
    - `icon-cascade` (added 2026-08-27): scans every `packages/core/src/*.css` file for a selector that declares `color: var(--lat-color-icon-*)` (anything but `color.icon.default`, Icon.css's own baseline) expecting an `<Icon>` child to inherit it via `currentColor`, but has no matching `<selector> .lat-icon { color: inherit; }` rescue rule alongside it. `Icon.css` sets `.lat-icon`'s own `color` unconditionally — a direct rule on the icon element itself always wins over an inherited value from a parent, regardless of the parent's specificity, so without the rescue rule the icon silently renders `color.icon.default` instead of the declared token. This is the structural gap `check-parity`/`check-component-bindings` can't close by design: both only verify a token is *referenced* somewhere in the CSS text, never that it actually wins the browser's cascade at render time. Found for real twice the same day (Search's clear icon, ChatInput's send icon) via direct computed-style checks in a live browser, not a token audit; Badge/Button already had the fix pattern, just never generalized into a mechanical check until now.
